...
const totalContributions = GAMES_JSON.reduce((contributionAccomulator, item) => {
    return contributionAccomulator + item.backers
}, 0)
...
const totalPledge = GAMES_JSON.reduce((pledgeAccomulator, item) => {
    return pledgeAccomulator + item.pledged
}, 0)

// set inner HTML using template literal
raisedCard.innerHTML = `$${totalPledge.toLocaleString()}`;
...
const totalRaisedGames = GAMES_JSON.reduce((sum, g) => sum + g.pledged, 0);
const totalGames  = GAMES_JSON.length;

const displayStr = `
  A total of $${totalRaisedGames.toLocaleString()} has been raised for ${totalGames} game${totalGames === 1 ? '' : 's'}. 
  Currently, ${unfundedCount} game${unfundedCount === 1 ? ' remains' : 's remain'} unfunded. 
  We need your help to fund these games! 
`; 
//another ternary operator could be used to use "this" instead of "these" when 1 game is remaining, but given the
// example in the PDF I decided to leave the "these" for both cases
